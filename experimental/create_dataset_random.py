...
def write_files(keys, level, output_dir, k, train_fraction, train_set):
    with open(os.path.join(output_dir, level, directory_for_ml_training,
        ...
        if train_set == 'True':
            fasta_db_handle = open(os.path.join(output_dir, level, directory_for_db_creation,
                                    str(keys), str(keys) + ".fasta"), 'a')
            taxid_db_handle = open(os.path.join(output_dir, directory_for_db_creation,
                                    str(keys), str(keys) + ".taxid"), 'a')

            for gens, locs in taxid2genomes[level][keys].items():
    ...
    if train_set == "True":
        ...
        taxid_db_handle.close()
                    # for i, kmer in enumerate(lst_kmers):
...
if __name__=="__main__":
    ...
    parser.add_argument('--output_dir', help='Path to sampled\
                        genomes pickle file')
