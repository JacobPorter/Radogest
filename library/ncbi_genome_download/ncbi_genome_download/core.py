...
def argument_parser(version=None, parser=None):
    """Create the argument parser for ncbi-genome-download."""
    if parser is None:
        parser = argparse.ArgumentParser()
    parser.add_argument('json', type=str,
                        default=NgdConfig.get_default('json'),
                        help=('The location of the '
                              'json file to store genome information. '
                              '(default: %(default)s)'))
...
def select_candidates(config):
    ...
    for group in config.group:
        for section in config.section:
            summary_file = get_summary(section, group, config.uri, config.use_cache)
            entries = parse_summary(summary_file)
            for entry in filter_entries(entries, config):
                entry['section'] = section
                download_candidates.append((entry, group))
    if download_candidates and config.json:
        json.dump(download_candidates, open(config.json, 'w'))
        #print(download_candidates[0])
...
def create_downloadjob(entry, domain, config):
    ...
    full_output_dir = create_dir(entry, entry['section'], domain, config.output)

    symlink_path = None
    if config.human_readable:
        symlink_path = create_readable_dir(entry, entry['section'], domain, config.output)
