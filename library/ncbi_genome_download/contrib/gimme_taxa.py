...
def main():
    ...
    if args.name:
        ...
        #args.taxid = ncbi.get_fuzzy_name_translation(args.name)[0]
    if args.verbose > 0:
        tax_dict = {}
    ...
    descendent_taxa_1 = set(ncbi.get_descendant_taxa(args.taxid,
                                               rank_limit='species'))
    descendent_taxa_2 = set(ncbi.get_descendant_taxa(args.taxid,
                                                     rank_limit='subspecies'))
    descendent_taxa = descendent_taxa_1.union(descendent_taxa_2)
                                               #collapse_subspecies=True)
    #sys.stderr.write(descendent_taxa + "\n")
