...
def get_rc_fasta(filename_input, filename_output, prob=0.5, remove=True):
    """
    ...
    for seq_id, seq_seq in input_iterator:
        ...
        if random.random() < prob:
            try:
                ...
                print(read_counter, write_counter, seq_id, seq_seq)
                raise KeyError
            add_id = "_RC"
...
def main():
    ...
    parser.add_argument("-p", "--prob", type=float,
                        ...
                        default=0.5)
    parser.add_argument("--no_remove", "-n",
                        help=("Do not remove sequences with Ns in them."),
                        action='store_false',
                        default=True)
    ...
    read_counter, write_counter = get_rc_fasta(args.fasta_filename,
                                               args.output, prob=args.prob,
                                               remove=args.no_remove)
    print("Records read: {}.  Records written: {}".format(read_counter,
                                                          write_counter),
          file=sys.stderr)
